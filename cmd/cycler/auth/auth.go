...
func NewRoundTripper(next http.RoundTripper,header http.Header) http.RoundTripper {
	if next == nil {
...
func (rt *HeaderRoundTripper) RoundTrip(req *http.Request) (resp *http.Response, err error) {
	defer func(begin time.Time) {
		...
		fmt.Printf(
			...
			req.Method, req.URL.Host, req.URL.Path, resp.StatusCode, err, time.Since(begin),
		)
		} else {
			fmt.Printf(
